...
// Seed watchlist so the scanner isn't empty on first load — mixes the
// Vietnam/Laos research from before with a few other worldwide options
// found under (or near) the $400 round-trip target.
const SEED_TRIP_DESTINATIONS = [
...
export default function PlannerDashboard() {
  ...
  // --- Trip scanner state ---
  const [tripDestinations, setTripDestinations] = useState(SEED_TRIP_DESTINATIONS);
  const [tripPriceLogs, setTripPriceLogs] = useState(SEED_TRIP_LOGS);
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("day-planner-tasks");
        if (result && result.value) {
          const parsed = JSON.parse(result.value);
          Object.keys(parsed).forEach((k) => {
            parsed[k] = (parsed[k] || []).map((t) => ({ category: "work", ...t }));
          });
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("training-log-entries");
        if (result && result.value) {
          setTrainingEntries(JSON.parse(result.value));
        }
      } catch (e) {
        // no existing data — fine
      } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        const destResult = await window.storage.get("trip-destinations");
        if (destResult && destResult.value) {
          setTripDestinations(JSON.parse(destResult.value));
        }
      } catch (e) {
        // no existing data — seed defaults already in state
      }
      try {
        const logResult = await window.storage.get("trip-price-logs");
        if (logResult && logResult.value) {
          setTripPriceLogs(JSON.parse(logResult.value));
        }
      } catch (e) {
        // no existing data — seed defaults already in state
      } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("dashboard-dark-mode");
        if (result && result.value) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("masters-deadlines");
        if (result && result.value) {
          setDeadlines(JSON.parse(result.value));
        }
      } catch (e) {
        // no existing data — fine
      } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("dashboard-quick-links");
        if (result && result.value) {
          setQuickLinks(JSON.parse(result.value));
        }
      } catch (e) {
        // no existing data — defaults already in state
      } finally {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("dashboard-notes");
        if (result && result.value) {
  ...
  const persistTasks = useCallback(async (next) => {
    try {
      const result = await window.storage.set("day-planner-tasks", JSON.stringify(next));
  ...
  useEffect(() => {
    if (loaded) persistTasks(tasksByDay);
  }, [tasksByDay, loaded, persistTasks]);

  const persistTraining = useCallback(async (next) => {
    try {
      const result = await window.storage.set("training-log-entries", JSON.stringify(next));
      if (!result) setTrainingError("Couldn't save — your changes may not persist.");
      else setTrainingError(null);
    } catch (e) {
  ...
  useEffect(() => {
    if (trainingLoaded) persistTraining(trainingEntries);
  }, [trainingEntries, trainingLoaded, persistTraining]);

  const persistQuickLinks = useCallback(async (next) => {
    try {
      const result = await window.storage.set("dashboard-quick-links", JSON.stringify(next));
      if (!result) setQuickLinksError("Couldn't save — your changes may not persist.");
      else setQuickLinksError(null);
    } catch (e) {
  ...
  useEffect(() => {
    if (quickLinksLoaded) persistQuickLinks(quickLinks);
  }, [quickLinks, quickLinksLoaded, persistQuickLinks]);
  ...
  function addQuickLink() {
    ...
    setQuickLinks((prev) => [...prev, { id: uid(), label, url }]);
  ...
  const persistNotes = useCallback(async (next) => {
    try {
      const result = await window.storage.set("dashboard-notes", JSON.stringify(next));
      if (!result) setNotesError("Couldn't save — your changes may not persist.");
  ...
  useEffect(() => {
    if (notesLoaded) persistNotes(notes);
  }, [notes, notesLoaded, persistNotes]);

  function addNote() {
    const text = noteDraft.text.trim();
    if (!text) return;
    setNotes((prev) => [
      { id: uid(), title: noteDraft.title.trim() || "Untitled", text, createdAt: Date.now() },
      ...prev,
    ]);
